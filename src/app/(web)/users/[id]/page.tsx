...
const UserDetails = (props: { params: { id: string } }) => {
  ...
  const [isRoomRatingVisible, setIsRoomRatingVisible] = useState(false);
  const [isSubmittingReview, setIsSubmittingReview] = useState(false);
  const [ratingValue, setRatingValue] = useState<number | null>(0);
  const [ratingText, setRatingText] = useState("");

  const reviewSunmitHandler = async () => {
    if (!ratingText.trim().length || !ratingValue)
      return toast.error("Please provide rating and review text");

    if (!roomId) return toast.error("Invalid Room Id");
    ...
    try {
      ...
      setIsRoomRatingVisible(false);
    }
  };

  const toggleRatingModal = () =>
    setIsRoomRatingVisible((prevState) => !prevState);
  ...
  return (
    <div className="container mx-auto px-2 md:px-4 py10">
      ...
      <RatingModal
        isOpen={isRoomRatingVisible!}
        ratingValue={ratingValue}
        setRatingValue={setRatingValue}
        ratingText={ratingText}
        setRatingText={setRatingText}
        isSubmittingReview={isSubmittingReview}
        reviewSunmitHandler={reviewSunmitHandler}
        toggleRatingModal={toggleRatingModal}
      />
      <BackDrop isOpen={isRoomRatingVisible!} />
    </div>
  );
};
